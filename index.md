...
\
`devtools``::`[`install_github`](https://devtools.r-lib.org/reference/install-deprecated.html)`(``"urswilke/crosstabser"``)`\
`devtools``::`[`install_gitlab`](https://devtools.r-lib.org/reference/install-deprecated.html)`(``"urswilke/crosstabser"``)`
...
\
`df`` ``<-`` ``tibble``::`[`tibble`](https://tibble.tidyverse.org/reference/tibble.html)`(`\
`  q1 ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``1``, ``2``, ``1``)`` ``|>`` ``haven``::`[`labelled`](https://haven.tidyverse.org/reference/labelled.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``Yes ``=`` ``1``, No ``=`` ``2``)``, label ``=`` ``"Super important question"``)``,`\
`  age ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``2``, ``1``, ``1``)`` ``|>`` ``haven``::`[`labelled`](https://haven.tidyverse.org/reference/labelled.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"18-39"`` ``=`` ``1``, ``"40+"`` ``=`` ``2``)``, label ``=`` ``"age"``)`\
`)`\
`df`\
`#> # A tibble: 3 × 2`\
`#>   q1        age      `\
...
\
`(``mapping_file`` ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`\
`  Questions ``=`` `[`data.frame`](https://rdrr.io/r/base/data.frame.html)`(`\
`    Type  ``=`` ``"cat"``,`\
`    RowVar ``=`` ``"q1"``,`\
`    Title ``=`` ``"The crosstab's title"`\
`  ``)``,`\
`  Macro ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``ColVar ``=`` ``"age"``)`\
`)``)`\
`#> $Questions`\
`#>   Type RowVar                Title`\
...
\
[`Tabula`](https://urswilke.codeberg.page/crosstabser/reference/Tabula.md)`$``new``(``df``, ``mapping_file``)`\
`` #> $`2` ``\
`` #> $`2`[[1]] ``\
