...
\
`m`` ``<-`` ``crosstabser``::`[`Tabula`](https://urswilke.codeberg.page/crosstabser/reference/Tabula.md)`$``new``(`\
`  dat ``=`` ``df``, `\
...
\
`m`` ``|>`` `[`print`](https://rdrr.io/r/base/print.html)`(``width ``=`` ``1e4``)`\
`` #> $`3 - _row_3: 1` ``\
...
`  Macro ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``ColVar ``=`` ``ColVar``)`\
`)`\
`m`` ``<-`` ``crosstabser``::`[`Tabula`](https://urswilke.codeberg.page/crosstabser/reference/Tabula.md)`$``new``(`\
`  dat ``=`` ``df``, `\
`  mapping_file ``=`` ``mapping_file`\
