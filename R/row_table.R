...
gen_row_table <- function(qtab) {
  ...
  if (!is.null(weight)) {
    ...
    weight_sign <- qtab$m$opts$ct$l_lexikon["cTabWeighted"]
...
row_table_body.qtab_type_mcg <- row_table_body.qtab_type_cat <- function(qtab) {
  ...
  row_table$RowTitle3 <- c(
    qtab$m$opts$ct$l_lexikon[["cTabAbs"]],
    qtab$m$opts$ct$l_lexikon[["cTabProz"]]
  ) |> rep(n_vals)
...
row_table_body.qtab_type_mw <- function(qtab) {
  ...
  row_title3 <- dplyr::coalesce(
    ...
    qtab$m$opts$ct$l_lexikon[[ctab_entry]]
  )
  n_valid_label <- dplyr::coalesce(
    qtab$p$df_stat_funs[["ValidN"]],
    qtab$m$opts$ct$l_lexikon[["cTabGueltig"]]
  )
  fun_decimals <- (
...
row_table_valid_cases.default <- function(qtab) {
  ...
  abs_text <- qtab$m$opts$ct$l_lexikon[["cTabAbs"]]
  percent_text <- qtab$m$opts$ct$l_lexikon[["cTabProz"]]
...
row_table_summary.qtab_type_cat <- function(qtab) {
  ...
  row_table$RowTitle1 <- paste0(qtab$m$opts$ct$l_lexikon[["cTabZsfg"]], " ")
...
row_table_invalid_vals.qtab_type_mcg <- row_table_invalid_vals.qtab_type_cat <- function(qtab) {
  ...
  row_table$row_type <- c("detail_freqs_invalid", "detail_perc_invalid") |> rep(n_vals)
...
row_table_invalid_vals.qtab_type_mdg <- function(qtab) {
  ...
  l_varlabs <- qtab$m$ditw$ct$dat_tab[invalid_vals] |> purrr::map(\(x) attr(x, "label", exact = TRUE))
  mdg_val <- qtab$p$MdgVal
...
row_table_no_entry.qtab_type_mdg <- function(qtab) {
  ...
  row_table <- empty_row_table()
  no_entry_text <- qtab$p$MdgMissLab %||% qtab$m$opts$ct$l_lexikon[["cTabNoEntry"]]
  abs_text <- qtab$m$opts$ct$l_lexikon[["cTabAbs"]]
  percent_text <- qtab$m$opts$ct$l_lexikon[["cTabProz"]]
  ...
  row_table$row_type <- c("detail_freqs_invalid", "detail_perc_invalid")
  row_table
